...
public class Auto_Crater_Sampling extends OpMode {
    ...
    public void loop(){
        ...
        switch (state) {
            ...
            case RETURNRIGHT:
                if (!isFinished) {
                    isFinished = robot.driveTrain.encoderDrive(DriveTrain.Direction.S, 0.25, 2.25);
                } else {
                    isFinished = false;
            ...
            case STRAFELEFTFROMRIGHT:
                if (!isFinished) {
                    isFinished = robot.driveTrain.encoderDrive(DriveTrain.Direction.W, 0.25, 12);
                } else {
                    isFinished = false;
            ...
            case STRAFELEFTFROMMID:
                if (!isFinished) {
                    isFinished = robot.driveTrain.encoderDrive(DriveTrain.Direction.W, 0.25, 7);
                } else {
                    isFinished = false;
                    state = State.TURNRIGHT;
                }
            ...
            case RETURNLEFT:
                if (!isFinished) {
                    isFinished = robot.driveTrain.encoderDrive(DriveTrain.Direction.S, 0.25, 3.25);
                } else {
                    isFinished = false;
            ...
            case STRAFETOENDPOINT:
                if (!isFinished) {
                    isFinished = robot.driveTrain.encoderDrive(DriveTrain.Direction.W, 0.25, 1.7                                                                                             );
                } else {
                    isFinished = false;
                    state = State.TURNRIGHT;
                }
            ...
            case ALIGN:
                if (!isFinished) {
                    isFinished = robot.driveTrain.encoderDrive(DriveTrain.Direction.S, 0.25, 5.75);
                } else {
                    isFinished = false;
...
/*            case TURNLEFT:
            ...
            case ROTATECOUNTERCLOCKWISE:
                if (!isFinished) {
                    ...
                    isFinished = false;
                    state = State.STRAFETOWALL;
                }
                break;
            case STRAFETOWALL:
                if (!isFinished) {
                    isFinished = robot.driveTrain.encoderDrive(DriveTrain.Direction.W, 0.5, 2.25);
                } else {
                    isFinished = false;
                    state = State.DRIVEONTOCRATER;
                }
                break;
            case DRIVEONTOCRATER:
                if (!isFinished) {
                    isFinished = robot.driveTrain.encoderDrive(DriveTrain.Direction.N, 0.3, 6.25);
                } else {
                    isFinished = false;
                    state = State.FINALE;
                }
